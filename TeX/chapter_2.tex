...
Пусть контейнер (\ref{container:eq}) представляет собой цепь Маркова 1-го порядка с вектором распределения вероятностей $\pi = (\frac{1}{2}, \frac{1}{2})$ и матрицей вероятностей одношаговых переходов
...
\end{equation}
Тогда на основании (\ref{ost00}) и (\ref{ost01}) остаточный член для энтропии будет равен:\\
\begin{gather*}
...
\section{Математическая модель последовательности на основании серий}
	...
	Исследование данной стегопоследовательности разбивается на два случая в зависимости от длины первой серии: первая серия длины 1; первая серия длины большей 1.
	
	Рассмотрим случай когда $y_1\neq y_2$:
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\ =\frac{1}{2}\biggl(\frac{1}{2}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1}\biggl)\biggl(\frac{1}{2}(1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s}\biggl) =\frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s};
	\end{gather*} 
	
	
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=1, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^{T+1}}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s+1};
	\end{gather*} 
	
	\begin{gather*}
		P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
		\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=1,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^{T+1}}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s+1};
	\end{gather*} 
	
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ...,y_{t+j}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0,...,\gamma_{t+j}=1, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\} =\\  =\frac{1}{2^{T+1}}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-2} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s}(1+\varepsilon^2), 1<j<s;
	\end{gather*} 
	
	\begin{gather*}
		P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
		\gamma_1=1, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1};
	\end{gather*} 
	
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=1,\gamma_{t+s+1}=0, ..., \gamma_T=1\}=\\  \frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1};
	\end{gather*} 
	...
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1};
	\end{gather*} 
	...
	\begin{gather*}
		P\{y_1=0, ..., y_t=0,y_{t+1}=1, ...,y_{t+j}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
		\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0,...,\gamma_{t+j}=1, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\} =\\  =\frac{1}{2^{T+1}}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1}(1+\varepsilon^2), 1<j<s;
	\end{gather*} 
	
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=1, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^T+1}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1};
	\end{gather*} 
	
	\begin{gather*}
	P\{y_1=0, ..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=1\}=\\   =\frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1};
	\end{gather*} 
	
	
	
	\begin{gather*}
	P\{y_1=0, ...,y_j=0,..., y_t=0,y_{t+1}=1, ..., y_{t+s}=1,y_{t+s+1}=0, ..., y_T=1|\\
	\gamma_1=0, ..., \gamma_j=1,..., \gamma_t=0,\gamma_{t+1}=0, ..., \gamma_{t+s}=0,\gamma_{t+s+1}=0, ..., \gamma_T=0\}=\\  =\frac{1}{2^T+1}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s+1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1}, 1<j<t;
	\end{gather*} 
	
	\begin{definition}
		Расстоянием Хемминга для последовательности $\gamma$ называется величина \cite{harin}:\newline
		 \begin{equation}
		 w(\gamma)=\sum\limits_{i=1}^{T}\gamma_i
		 \end{equation}
	\end{definition}
	...
	\end{lemma}
	\begin{proof}
		Асимптотическая оценка 1-го порядка при $\delta \rightarrow 0$ вероятности равна:
		\begin{gather*}
		P\{Y\}=P\{Y|w(\gamma)=0\} + \delta P\{Y|w(\gamma)=1\}+O(\delta^2);
		\end{gather*}
		На основании анализа вероятностей стегопоследовательности получаем:
		...
		\bigl(-\frac{3}{2}\varepsilon^4-\frac{5}{2}\varepsilon^3-\frac{5}{2}\varepsilon^2+\frac{3}{2}\varepsilon + 7\bigl);
		\end{gather*}
		Отсюда следует:
		
	\begin{gather*}
	P\{Y\}=P\{Y|w(\gamma)=0\} + \delta P\{Y|w(\gamma)=1\}+O(\delta^2)=\\ \frac{1}{2^{T-1}}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-1} +\\ \delta \biggl( \frac{1}{2^T}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1-2} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-2}\bigl(-\frac{3}{2}\varepsilon^4-\frac{5}{2}\varepsilon^3-\frac{5}{2}\varepsilon^2+\frac{3}{2}\varepsilon + 7\bigl) \biggl)+O(\delta^2) = \\
	\frac{1}{2^T-1}(1+\varepsilon)^{T-\sum\limits_{s=1}^{k}\nu_s-1-2} (1-\varepsilon)^{\sum\limits_{s=1}^{k}\nu_s-2}\\
	\biggl((1+\varepsilon)(1-\varepsilon)-\delta\bigl(\frac{3}{4}\varepsilon^4+\frac{5}{4}\varepsilon^3+\frac{5}{4}\varepsilon^2-\frac{3}{4}\varepsilon - \frac{7}{2}\bigl)\biggl)+O(\delta^2)
	\end{gather*}
	\end{proof}
...
Для использования ЛДА признаки должны быть непрерывными величинами, иначе следует использовать анализ соответствий.\cite{lda}
\subsection{Линейный дискриминантный анализ для случая двух классов}
Для каждого образца объекта или события с известным классом $y$ рассматривается набор наблюдений $x$ (называемых еще признаками, переменными или измерениями). Набор таких образцов называется обучающей выборкой (или набором обучения, обучением). Задачи классификации состоит в том, чтобы построить хороший прогноз класса y для всякого так же распределенного объекта (не обязательно содержащегося в обучающей выборке), имея только наблюдения $x$.\newline
...
\begin{equation}
Y^q(u,v)=\lceil\frac{Y(u,v)}{q(u,v)}\rceil, 
\end{equation}
